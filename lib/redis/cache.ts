...
export async function getFromCache<T>(subPath: string, cacheKey: string): Promise<T | null> {
  try {
...
export async function saveToCache<T>(
  ...
  try {
    ...
    await redis.set(redisKey, JSON.stringify(wrappedData), ttlSeconds ? { ex: ttlSeconds } : undefined);
  } catch (error) {
