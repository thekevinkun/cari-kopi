...
export default async function handler(
  ...
  const cacheKey = shortAddress
    ? slugify(String(shortAddress))
    : `${lat},${lng}`;
  const pageNumber = parseInt(page as string, 10) || 1;
  const pageKey = `${cacheKey}:page:${pageNumber}`;

  // Try to fetch this page from cache
  const cached = await getFromCache<{ results: PlaceResponse[] }>(
    subPath,
    pageKey
  );
  if (cached) {
    ...
    const meta = await getFromCache<{
      totalPages: number;
      totalResults: number;
      lastUpdated: string;
      locationKey: string;
    }>(subPath, `${cacheKey}:meta`);
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Failed to fetch and cache coffee shops." });
  }
}
